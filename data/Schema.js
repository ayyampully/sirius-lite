...
module.exports = function(mongoose, _) {
	...
	publicAPI.getProjectDetails = function(projectid, callback, isOverview) {
		...
		var user = Projects.find({projectid:projectid}, function findProjects(err, projects){
			if (err) {
				// No valid user
				callback(false);
			} else {
				if(isOverview){
					...
					Tickets.find({'project.id':projectid}, function findProjects(err, tickets){
						if (err) {
							// No valid user
							callback(false);
						} else {
	...
	publicAPI.getTicketDetails = function(ticketid, projectid, callback){
		//this.getProjectDetails(projectid,(data)=>{
			Tickets.find({'ticketid':ticketid, 'project.id':projectid}, function findProjects(err, ticket){
				if (err) {
					// No tickets
					callback(false);
				} else {
	...
	publicAPI.updateTicket = function(ticketid, ticketupdate, callback){
		...
		Tickets.findOne({'ticketid':ticketid}, function findProjects(err, ticket){
				if (err) {
					// No tickets
		...
		/* Tickets.update({'ticketid':ticketid}, ticketupdate, function findProjects(err, ticket){
				if (err) {
					// No tickets
					callback(false);
				} else {
	...
	publicAPI.addComment = (commentObj, callback)=>{
		...
		Tickets.findOne({'ticketid':commentObj.ticketid}, function findProjects(err, ticket){
			if (err) {
				// No tickets
